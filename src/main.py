...
def load_config():
	...
	with open('storage/main_config.json', 'r', encoding='utf-8') as file:
...
def save_config():
	...
	with open('storage/main_config.json', 'w', encoding='utf-8') as file:
		json.dump(
		    {
		        'active_extensions': extensions,
		        'unactive_extensions': deactivated_extensions
		    },
		    file,
		    sort_keys=True,
		    indent=4
...
if __name__ == '__main__':
	if not os.path.exists('storage'):
		...
		mkdir('storage')
	if not os.path.isfile('storage/main_config.json'):
