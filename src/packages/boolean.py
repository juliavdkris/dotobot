...
class Boolean(commands.Cog):
	...
	async def set(self, ctx, name: str, *, expr: str):
		...
		log.debug(
		    f'Succesfully set \'{func["name"]}\' to statement: \'{func["stmt"]}\' using expression: \'{func["expr"]}\'. Collected variables: {", ".join(func["bvar"])} and nested functions: {", ".join(func["nfnc"])}'
		)
		...
		if target == 'all':
			...
			if 'insert' in ctx.message.content.split():
				try:
					...
					log.error(
					    f'User \'{ctx.author.name}\' provided functions containing non-existant nested functions for \'!b view\''
					)
	...
	async def table(self, ctx, *, targets: str):
		...
		for target in targets.split():
			...
			try:
				...
				log.error(
				    f'User \'{ctx.author.name}\' provided functions containing non-existant nested functions for \'!b table\''
				)
				await ctx.send(f'Function contains non-existant nested functions')
				return
			...
			if len(func['bvar']) > len(superset):
				...
				if func['bvar'] != superset:
					log.error(
					    f'User \'{ctx.author.name}\' provided targets containing conflicting variables for \'!b table\''
					)
	...
	async def kmap(self, ctx, target: str):
		...
		try:
			...
			log.error(
			    f'User \'{ctx.author.name}\' provided functions containing non-existant nested functions for \'!b kmap\''
			)
			await ctx.send(f'Function contains non-existant nested functions')
			return
